...
ml_models/*.json) directly from disk; they never touch PostgreSQL for the
ML data itself and never load a trained model (.pkl) file. All routes
except /health are PAYER-only, so each test (other than test_health) logs
in as PAYER first via an isolated in-memory SQLite database dedicated to
the auth check. Tests run against the real generated artifacts already
present in this repository and do not modify them. The artifact-unavailable
test safely substitutes the artifact directory via mocking so it exercises
the not-found error path without touching, moving, or deleting any real
project artifact.
"""
...
class MlResultsApiTestCase(unittest.TestCase):
    ...
    def _login_as_payer(self) -> None:
        ...
        response = self.client.post(
            "/api/auth/login",
            json={"email": "payer-mlresults@example.com", "password": "password123"},
        )
    ...
    def test_risk_predictions_rejects_unauthenticated(self) -> None:
        response = self.client.get("/api/ml/risk-predictions")
        self.assertEqual(response.status_code, 401)

    def test_risk_predictions_rejects_patient_role(self) -> None:
        with patch("backend.services.auth_service.session_scope", self._mock_session_scope):
            self.client.post(
                "/api/auth/register",
                json={"email": "patient-mlresults@example.com", "password": "password123", "role": "PATIENT"},
            )
            self.client.post(
                "/api/auth/login",
                json={"email": "patient-mlresults@example.com", "password": "password123"},
            )
            response = self.client.get("/api/ml/risk-predictions")
